...
class Build_MIMIC_IV_CXR(Sampler):
    def __init__(
        ...
        super().__init__()
        assert split in ["silver", "gold"]
        self.split = split  # silver (train + valid) or gold (test)
    ...
    def build_admission_table(self):
        ...
        if self.timeshift:
            ...
            random.seed(0)  # NOTE: fix the random seed
            _NUM_CUR_PATTIENTS = 40 if self.split == "gold" else 80  # 10% of the patients (400/800 patients)
